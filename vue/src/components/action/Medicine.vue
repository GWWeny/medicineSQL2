  ...
  methods: {
    handleSearch() {
      ...
      request.get('/medicine/search', { params: query })
          ...
          .catch(error => {
            ...
            // 判断错误类型，并显示更详细的错误信息
            if (error.response) {
              // 请求已发出，且服务器返回了非 2xx 的状态码
              const status = error.response.status;
              const message = error.response.data ? error.response.data.message || '请求失败，请稍后再试' : '服务器异常';
              this.$message.error(`请求失败，错误代码: ${status}, 错误信息: ${message}`);
            } else if (error.request) {
              // 请求已发出，但没有收到响应
              this.$message.error('请求未收到响应，请检查网络连接');
            } else {
              // 发生了错误，触发了错误的请求设置
              this.$message.error('请求配置出错: ' + error.message);
            }
    ...
    saveMedicine() {
      ...
      request[method](url, this.currentMedicine)
          .then(response => {
            // 根据 response 进行处理（假设 response 里有 code 字段表示操作结果）
            if (response && response.code === '200') {
              this.$message.success(this.isEditing ? '药品更新成功' : '药品添加成功');
            } else {
              this.$message.error('操作失败: ' + response.message || '请稍后再试');
            }
          ...
          .catch(error => {
            ...
            this.$message.error('保存药品失败，请稍后重试');
          });
    }
    ,
    // 编辑药品信息
    ...
    deleteMedicine(mno) {
      ...
      this.$confirm('确认删除此药品吗?', '警告', {
        ...
        request.delete(`/medicine/delete/${mno}`)
            .then(response => {
              // 假设服务器返回的 response 里有 code 字段
              if (response.code === '200') {
                // 删除成功后刷新药品列表
                this.fetchMedicines();
                this.$message.success('药品删除成功');
              } else {
                // 如果没有返回成功的 code，显示错误消息
                this.$message.error('删除失败: ' + (response.message || '请稍后重试'));
              }
            })
            .catch(error => {
              // 如果请求出错，显示错误提示
