...
import './dengl-normal'
import Agency from "@/components/action/Agency.vue";
  ...
  methods: {
    handleSearch() {
      ...
      request.get('/client/search', {params: query})
          ...
          .catch(error => {
            this.loading = false;
            console.error('查询失败:', error); // 记录错误信息
    ...
    fetchclient() {
      request.get('/client/findAll')
          ...
          .catch(error => {
            console.error('获取客户列表失败:', error); // 记录错误
            this.$message.error('获取客户数据失败，请稍后再试');
            this.clients = [];
          });
    }
    ,
    ...
    saveclient() {
      ...
      request[method](url, this.currentclient)
          .then(response => { // 处理 response
            if (response.code === '200') {
              this.$message.success('保存成功'); // 如果返回成功，显示成功提示
            } else {
              this.$message.error('保存失败：' + response.msg); // 如果返回失败，显示失败提示
            }
    ...
    deleteclient(cno) {
      this.$confirm('确认删除此经办人吗?', '警告', {
        ...
        request.delete(`/client/delete/${cno}`)
            .then(response => { // 处理 response
              if (response.code === '200') { // 假设返回的 response 包含一个 code 字段
                this.fetchclient();
                this.$message.success('删除成功');
              } else {
                this.$message.error('删除失败: ' + response.msg); // 如果删除失败
              }
            })
            .catch(error => {
